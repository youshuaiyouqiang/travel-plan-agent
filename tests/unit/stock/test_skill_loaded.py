...
def test_yaml_tools_match_port_methods():
    ...
    assert len(tools) == 15, f"期望 15 个工具，实际 {len(tools)}: {tools}"
...
def test_daily_session_excludes_correlation_tool():
    ...
    # 日复盘 14 工具，周复盘 15 工具
    assert len(daily_tools) == 14, f"日复盘应 14 个工具，实际 {len(daily_tools)}: {daily_tools}"
    assert len(weekly_tools) == 15, f"周复盘应 15 个工具，实际 {len(weekly_tools)}: {weekly_tools}"
