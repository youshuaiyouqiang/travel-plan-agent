...
def test_registry_has_exactly_20_versions():
    """注册表必须恰好包含 20 个迁移。"""
    assert len(MIGRATIONS) == 20


def test_registry_versions_are_1_to_20_continuous():
    """版本号必须连续 1..20。"""
...
def test_empty_db_upgrades_to_version_20(tmp_db):
    """空库 init_db 后 current_version 必须为 20。"""
    init_db()
...
def test_all_20_versions_recorded_in_schema_migrations(tmp_db):
    """schema_migrations 表必须记录全部 20 个版本。"""
    init_db()
    conn = get_connection()
...
def test_downgrade_from_20_to_15_then_upgrade_restores_20(tmp_db):
    """从版本 20 降级到 15，再升级应恢复到 20。"""
    init_db()
    assert get_migration_status()["current_version"] == 20

    # 降级到 15（保留 1..15，删除 16..20）
    downgrade(15)
    status_after_downgrade = get_migration_status()
    assert status_after_downgrade["current_version"] == 15
    assert status_after_downgrade["pending_count"] == 5

    # 再升级
    init_db()
    status_after_upgrade = get_migration_status()
    assert status_after_upgrade["current_version"] == 20
    assert status_after_upgrade["pending_count"] == 0


def test_downgrade_to_0_then_full_upgrade_restores_20(tmp_db):
    """从版本 20 全部降级到 0，再全量升级应恢复到 20。"""
    init_db()
    assert get_migration_status()["current_version"] == 20
