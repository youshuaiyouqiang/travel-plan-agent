"""MarketIndexFetcherAdapter——把 market_index_fetcher.run 适配为 Fetcher 协议。

设计要点（与 LimitFetcherAdapter 同构，AGENTS.md §8.1）：
...
class MarketIndexFetcherAdapter:
    """大盘指数 fetcher 适配器——实现 Fetcher 协议。

    复用 infrastructure.stock.market_index_fetcher.run 的逻辑；
    通过 akshare 拉数据，缓存到 CacheWritePort。
    """

    name = "market_index_fetcher"
    ...
    async def run(self, *, trade_date: str, repo: CacheWritePort) -> int:
        ...
        """
        from infrastructure.stock.market_index_fetcher import run as fetcher_run
